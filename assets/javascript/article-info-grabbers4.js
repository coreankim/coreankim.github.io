...
function authorGrabber(articleObj, hasAuthor) {
	...
	var authors = {
		...
		firstAuthorCountry: "N/A",
		firstAuthorState: "N/A",
		firstAuthorDepartment: "N/A",
		count: 0
	};
					...
					if (j === 0) {
						for (var k in jthAuthorObj) {
							if (k === "AffiliationInfo") {
								...
								// console.log(jthAuthorObj[k], jthAuthorObj[k].Affiliation)
								console.log(articleObj["Article"]["ArticleTitle"])
								var affArray = jthAuthorObj[k]["Affiliation"].split(",");
								var affCountry = affArray[affArray.length-1].split(".")[0].trim();
								authors["firstAuthorCountry"] = affCountry;
								if (affCountry === "USA") {
									if (affArray.length > 1) {
										authors["firstAuthorState"] = affArray[affArray.length-2].split(" ")[1].trim();
									};
									if (affArray.length > 3) {
										authors["firstAuthorDepartment"] = affArray[affArray.length-4].trim();
									};
								};
							};
						};
			...
			if (hasMultipleAuthors === false) {
				...
				for (var x in articleObj.Article.AuthorList.Author) {
					if (x === "AffiliationInfo") {
						...
						var affArray = authors.firstAuthorAffiliation.split(",");
						var affCountry = affArray[affArray.length-1].split(".")[0].trim();
						authors.firstAuthorCountry = affCountry;
						if (affCountry === "USA") {
							if (affArray.length > 1) {
